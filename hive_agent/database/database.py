...
class DatabaseManager:
    ...
    async def create_table(self, table_name: str, columns: Dict[str, str]):
        ...
        try:
            ...
            table_definition = TableDefinition(table_name=table_name, columns=columns_json)
    ...
    async def insert_data(self, table_name: str, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Data inserted into table {table_name} successfully, id: {instance.id}")
            return instance
        except SQLAlchemyError as e:
            await self.db.rollback()
    ...
    async def update_data(self, table_name: str, row_id: int, new_data: Dict[str, Any]):
        logger.info(f"Updating data in table {table_name} for id {row_id} with new data: {new_data}")
        try:
            columns_json = await self.get_table_definition(table_name)
            if not columns_json:
                raise ValueError(f"Table {table_name} does not exist.")
            ...
            if instance:
                ...
                await self.db.commit()
                await self.db.refresh(instance)
                logger.info(f"Data in table {table_name} for id {row_id} updated successfully.")
            else:
                logger.warning(f"No data found with id {row_id} in table {table_name}.")
                raise ValueError(f"No data found with id {row_id} in table {table_name}.")
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error(f"Error updating data in table {table_name} for id {row_id}: {str(e)}")
    ...
    async def delete_data(self, table_name: str, row_id: int):
        ...
        try:
            ...
            if instance:
                ...
                await self.db.commit()
                logger.info(f"Data deleted from table {table_name} for id {row_id} successfully.")
            else:
                logger.warning(f"No data found with id {row_id} in table {table_name}.")
                raise ValueError(f"No data found with id {row_id} in table {table_name}.")
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error(f"Error deleting data from table {table_name} for id {row_id}: {str(e)}")
